...
CryptoProviderParams): Promise<CryptoProvider> => {
  ...
  const prepareShelleyWitness = async (
    ...
    const publicKey = (await deriveXpub(path)).slice(0, 32) // TODO: use xpub2Pub helper
  ...
  const prepareByronWitness = async (
    ...
    const publicKey = xpub.slice(0, 32) // TODO: use xpub2Pub helper
    const chainCode = xpub.slice(32, 64) // TODO: use xpub2ChainCode helper
